#!/usr/bin/env python3
# ============================================================================
# extract_nepal_3p.py — 从 nepal_kukri_viewmodel.gltf 提取"纯刀"静态 3P 模型
# - 烘焙 bind-pose 顶点（GIBM × 顶点加权），排除 hand__hands（手）网格
# - 保留刀身 3 网格（ref-1/ref-2/ref-3）多材质
# - 输出 fp_viewmodel/nepal/nepal_world_static.gltf + .bin + textures/
# ============================================================================
import json, struct, os, shutil

SRC = "nepal_kukri_viewmodel.gltf"
OUT_DIR = "."
OUT_GLTF = "nepal_world_static.gltf"
OUT_BIN = "nepal_world_static.bin"
...
out_gltf = {
    "asset":{"version":"2.0","generator":"nepal_3p_static"},
    "scene":0,
    "scenes":[{"nodes":[0]}],
    "nodes":[{"name":"nepal_world_static","mesh":0}],
    "meshes":[{"name":"nepal_kukri","primitives":prims_out}],
    "materials":out_mats,
...
# 刀身 bbox 摘要
for wpos, _, _, _, _ in out_prims:
    mn=[min(p[i] for p in wpos) for i in range(3)]
    mx=[max(p[i] for p in wpos) for i in range(3)]
    print("  bbox center=([%.4f,%.4f,%.4f]) size=([%.4f,%.4f,%.4f])" % (
